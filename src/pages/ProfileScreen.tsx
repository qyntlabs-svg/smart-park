...
const ProfileScreen = () => {
  ...
  const menuItems = [
    { icon: Car, label: "My Vehicles", action: () => navigate("/vehicles") },
    {
      ...
      // Wire to Wallet screen which already exists — surfaces saved UPI IDs.
    ...
    },
    {
      ...
      onToggle: (v: boolean) => {
        setNotifications(v);
        toast.success(
          v ? "Push notifications enabled" : "Push notifications muted",
        );
      },
    },
    {
  ...
  return (
    <AnimatedPage className="min-h-[100dvh] w-full max-w-md mx-auto bg-background flex flex-col">
      ...
      {/* Logout confirmation */}
      {/** Kept inline so we don't shift the entire screen. */}
      {showLogout && (
