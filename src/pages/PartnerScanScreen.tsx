...
const PartnerScanScreen = () => {
  ...
  const startScanning = async () => {
    ...
    try {
      ...
      // Step 2: Enable transparent UI so the camera preview shows through
      document.body.classList.add("scanner-active");
      document.documentElement.classList.add("scanner-active");

      // Step 3: Listen for scan results
      scanListenerRef.current = await BarcodeScanner.addListener(
        "barcodesScanned",
        async (event) => {
          const value =
          ...
          // Stop as soon as we have one result to avoid duplicate fires
          if (scanListenerRef.current) {
            await scanListenerRef.current.remove();
            scanListenerRef.current = null;
          }
