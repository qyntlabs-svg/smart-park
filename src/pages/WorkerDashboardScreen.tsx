    ...
    <AnimatedPage className="min-h-[100dvh] w-full max-w-md mx-auto bg-background flex flex-col">
      <header className="px-5 pt-safe pb-4 bg-card border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => navigate("/worker/notifications")} className="touch-target relative">
        ...
        {/* W-09 availability pill */}
        <button
          onClick={() => navigate("/worker/availability")}
          className="mt-3 w-full h-10 rounded-xl bg-secondary flex items-center gap-2 px-3 text-body-sm font-semibold text-foreground"
        >
          <Circle
            className={`w-3 h-3 ${worker.status === "approved" ? "fill-success text-success" : "fill-muted-foreground text-muted-foreground"}`}
          />
          <span className="capitalize">
            {worker.status === "approved" ? "Online" : worker.status.replace("_", " ")}
          </span>
        ...
        {list.map((b) => {
          ...
          return (
            <div key={b.id} className="p-4 rounded-2xl bg-card border border-border space-y-3">
              ...
              <button
                onClick={() => navigate(`/worker/jobs/${b.id}`)}
                className="w-full h-9 rounded-lg bg-secondary text-foreground text-caption font-semibold"
              >
                Open job detail →
              </button>
            </div>
...
const QuickTile = ({
  ...
  <button
    onClick={onClick}
    className="relative p-3 rounded-xl bg-card border border-border text-center"
  >
    <Icon className="w-4 h-4 mx-auto text-primary" />
    <p className="text-caption font-semibold text-foreground mt-1">{label}</p>
    {badge && badge > 0 && (
